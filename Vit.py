...
class VisionTransformer(nn.Module):  # Transformer-branch
    ...
    def forward(self, x, skip_x, text, reconstruct=False):  # x = torch.Size([4, 64, 224, 224]), skip_x = torch.Size([4, 64, 224, 224]), text = torch.Size([4, 10, 64])二层([4, 128, 112, 112])([4, 196, 64])([4, 10, 128])

        # print(x.size())
        # print(skip_x.size())
        # print(text.size())
        if not reconstruct:  # 64.先来这
            # print("not")
            x = self.embeddings(x)  # torch.Size([4, 196, 64]) torch.Size([4, 196, 128])
            # print(x.size())
            if self.dim == 64:  # 64.来这
                # print("dim64")
                x = x+self.CTBN3(text)  # [B, num_patches, embed_dim] 都为torch.Size([4, 196, 64]) torch.Size([4, 196, 128])
                # print(self.CTBN3(text).size())
                # print(x.size())
            x = self.Encoder_blocks(x)  # torch.Size([4, 196, 64])
            # print(x.size())
        else:
            # print("else")
        ...
        if (self.dim == 64 and not reconstruct) or (self.dim == 512 and reconstruct):
            return x
        elif not reconstruct:  # 128来这
            # print("elif not reconstruct")
            x = x.transpose(1, 2)  # [B, embed_dim, num_patches] torch.Size([4, 128, 196])
            # print(x.size())
            x = self.CTBN(x)  # [B, embed_dim//2, num_patches] torch.Size([4, 64, 196])
            # print(x.size())
            x = x.transpose(1, 2)  # [B, num_patches, embed_dim//2] torch.Size([4, 196, 64])
            # print(x.size())
            y = torch.cat([x, skip_x], dim=2)  # [B, num_patches, embed_dim] torch.Size([4, 196, 128])
            # print(y.size())
            return y
        elif reconstruct:
